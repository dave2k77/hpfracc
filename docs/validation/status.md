...
| Non-uniform / graded-mesh Caputo solver (`NonUniformPredictorCorrector`) | Validated that the variable-step fractional Adams-Bashforth-Moulton solver reduces exactly to the uniform `PredictorCorrector` on an equispaced grid, matches the Mittag-Leffler solution on a graded mesh and refines, and restores convergence the uniform grid loses to the `t^alpha` origin singularity (graded global max-norm error decreases monotonically under refinement and beats a uniform grid of equal node count at the finest level), with finite initial-state / parameter gradients, jit consistency, and an `n`-independent graph. Explicit, full-history; a-posteriori adaptive stepping and SOE acceleration are future. | Unit tests in `tests/unit/test_nonuniform_solver.py`. |
| Implicit Caputo solver (`ImplicitPredictorCorrector`) | Validated that the implicit fractional Adams-Moulton solver (per-step fixed-iteration Newton) matches the Mittag-Leffler solution at mild rates and refines; on a stiff linear problem where the explicit `PredictorCorrector` diverges it stays bounded and tracks the decay; it is more accurate than the explicit method in the moderately-stiff regime (vs a fine-grid self-refinement reference); the result is Newton-iteration-independent for linear dynamics; and the graph size is independent of `n` with finite initial-state / parameter gradients. Full-history, fixed-step, uniform-grid; adaptive stepping is future. | Unit tests in `tests/unit/test_implicit_solver.py`. |
| Adaptive-step Caputo solver (`AdaptivePredictorCorrector`) | Validated that the a-posteriori adaptive solver, which chooses its mesh from a step-doubling (Richardson) local-error estimate, tracks tolerance (the error against a Mittag-Leffler reference decreases as `rtol` is tightened) and earns its keep on the `t^alpha` origin singularity (it clusters nodes near `t=0` and reaches an accuracy a uniform fixed-step grid of several times as many nodes cannot, because the uniform global error stalls). Because it advances with the single full step (no local extrapolation), the realized trajectory is identical to `NonUniformPredictorCorrector` on the realized mesh, so the frozen-mesh gradient equals that solver's gradient there and is finite-difference checked (the step-size controller and accept/reject decisions are not differentiated); `jit` is consistent and the masked-scan graph is bounded by `max_steps` independently of the problem. Correct full-history `O(N^2)` reference; SOE-accelerated history (cheap rejections), dense output, and an adaptive+implicit combination are future. | Unit tests in `tests/unit/test_adaptive_solver.py`. |
